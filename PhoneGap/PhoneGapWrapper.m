...
- (void)webView:(UIWebView *)webView didFailLoadWithError:(NSError *)error {
    ...
    //NSString *errorMessage = [NSString stringWithFormat:@"Failed to load webpage with error: %@", [error localizedDescription]];
...
- (BOOL) execute:(InvokedUrlCommand*)command
{
	...
	if ([obj respondsToSelector:NSSelectorFromString(fullMethodName)]) {
		[obj performSelector:NSSelectorFromString(fullMethodName) withObject:command.arguments withObject:command.options];
         NSLog(@"Selector : %@ added for Class : %@",fullMethodName,command.className);
	}
	else {
